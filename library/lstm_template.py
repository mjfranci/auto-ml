...
class LSTMNet(nn.Module):
    """
    Standard customizable LSTM architecture for sequential data.
    """
...
def parse_args():
    parser = argparse.ArgumentParser(description="AutoML LSTM Template")
    parser.add_argument("--run-name", type=str, default="lstm-sweep-run", help="Name of this run")
...
def main():
    ...
    # 1. Prepare Data
    print("[LSTM] Generating synthetic sequence data...")
    ...
    print(f"[LSTM] Class imbalance ratio: {imbalance_ratio:.2f}:1")
    primary_metric = "accuracy" if imbalance_ratio <= 3.0 else "f1"

    # 2. Hardware Optimization
    hw_opt = HardwareOptimizer(target_batch_size=args.batch_size)
    opts = hw_opt.suggest_optimal_parameters(model_param_count=12000)
    print(f"[LSTM] Hardware parameters:")
    ...
    # 3. Model Definition
    model = LSTMNet(input_size=1, hidden_size=32, num_layers=1, num_classes=2).to(hw_opt.device)
    ...
    hyperparameters = {
        ...
        "inference_latency_ms": 3.2 # mock latency
    }
    tracker = MLTracker(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if success:
            ...
            print(f"[LSTM] Recovering from OOM. Halving batch size to {hw_opt.current_batch_size}")
        ...
        if triage and triage.get("status") in ("KILL", "SUSPEND"):
            print(f"[LSTM] AutoML stopping logic fired: {triage.get('status')}")
    ...
    tracker.log_verdict(
        status="CONVERGED",
        message=f"LSTM training completed. Val Accuracy: {val_acc:.4f}",
